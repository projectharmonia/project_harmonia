...
impl Plugin for HighlightingPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<Highlighting>()
            .add_observer(show)
            .add_observer(hide)
            .add_observer(init_scene)
            .add_observer(enable)
            .add_observer(disable)
            .add_systems(OnEnter(BuildingMode::Objects), highlight_objects)
            .add_systems(OnEnter(CityMode::Objects), highlight_objects)
            .add_systems(OnEnter(FamilyMode::Life), highlight_actors_and_objects)
            .add_systems(OnExit(BuildingMode::Objects), disable_highlighting)
            .add_systems(OnExit(CityMode::Objects), disable_highlighting)
            .add_systems(OnExit(FamilyMode::Life), disable_highlighting);
    }
}
