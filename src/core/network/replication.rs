...
impl Plugin for ReplicationPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<TickAck>()
            ...
            .add_system(Self::tick_acks_cleanup_system.run_if_resource_exists::<RenetServer>())
...
impl ReplicationPlugin {
    ...
    fn tick_ack_sending_system(tick_ack: Res<TickAck>, mut client: ResMut<RenetClient>) {
    ...
    fn tick_acks_receiving_system(
        ...
        for client_id in server.clients_id() {
            if let Some(tick_ack) = receive_tick_ack(&mut server, client_id) {
...
fn receive_tick_ack(server: &mut RenetServer, client_id: u64) -> Option<TickAck> {
    ...
    received_acks.into_iter().max_by_key(|tick_ack| tick_ack.0)
}
